...
/// adapter does not own, close, copy, or move the source iterator.
...
class LimitedInputIterator {
 public:
  ...
  byte read() {
    if (status() != kOk) return byte{0};
    if (remaining_ == 0) {
      status_ = kEndOfStream;
      return byte{0};
    }
    byte result = input_.read();
    if (input_.status() != kOk) return byte{0};
    --remaining_;
    return result;
  }
  ...
  /// Returns the local boundary failure or the source iterator status.
  Status status() const { return status_ == kOk ? input_.status() : status_; }
