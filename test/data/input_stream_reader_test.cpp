...
InputStreamReader NewReader(const byte* begin, const byte* end) {
  return InputStreamReader(
      std::unique_ptr<InputStream>(new MemoryInputStream(begin, end)));
}
...
TEST(Reader, NullOwnedStreamIsClosed) {
  ...
  InputStreamReader reader(std::move(input));
  EXPECT_EQ(kClosed, reader.status());
  reader.readBeU16();
  EXPECT_EQ(kClosed, reader.status());
}
...
TEST(Reader, FloatEndian) {
  ...
  InputStreamReader be_reader = NewReader(be, be + 4);
  EXPECT_EQ(expected, be_reader.readBeFloat());

  InputStreamReader le_reader = NewReader(le, le + 4);
...
TEST(Reader, DoubleEndian) {
  ...
  InputStreamReader be_reader = NewReader(be, be + 8);
  EXPECT_EQ(expected, be_reader.readBeDouble());

  InputStreamReader le_reader = NewReader(le, le + 8);
...
TEST(Reader, VarU64_150) {
  ...
  InputStreamReader reader = NewReader(d, d + 2);
...
TEST(Read, ShortCString) {
  ...
  InputStreamReader reader = NewReader(in, in + 4);
  char buf[] = {9, 9, 9, 9, 9};
...
TEST(Read, ShortCStringUnderBuf) {
  ...
  InputStreamReader reader = NewReader(in, in + 5);
  char buf[] = {9, 9, 9, 9, 9};
  EXPECT_EQ(0, reader.readCString(buf, 2));
  EXPECT_TRUE(reader.hasDataError());
  InputStreamReader truncated = NewReader(in, in + 5);
