...
TEST_F(ArduinoFs, StatNonExistentFileWithFilesInPath) {
  CreateTextFile("/a/t.txt", "foo");

  Stat s = mount().stat("/a/t.txt/foo");
  EXPECT_EQ(kNotFound, s.status());  // kNotDirectory would have been nicer.
  EXPECT_FALSE(s.isFile());
  EXPECT_FALSE(s.isDirectory());
  EXPECT_EQ(0, s.size());
}
...
TEST_F(ArduinoFs, UnsuccessfulRemove) {
  ...
  EXPECT_EQ(
      kNotFound,
      mount().remove("/a/t.txt/foo"));  // kNotDirectory would have been nicer.
}
...
TEST_F(ArduinoFs, SuccessfulRmdir) {
...
TEST_F(ArduinoFs, UnsuccessfulRmdir) {
  CreateTextFile("/a/b/c/t.txt", "foo");
  ...
  EXPECT_EQ(kNotFound,
            mount().rmdir(
                "/a/b/c/t.txt/foo"));  // kNotDirectory would have been nicer.
}

TEST_F(ArduinoFs, SuccessfulRename) {
...
TEST_F(ArduinoFs, SuccessfulMove) {
  CreateTextFile("/a/b/c/foo.txt", "foo");
...
TEST_F(ArduinoFs, SuccessfulMoveDir) {
  CreateTextFile("/a/b/c/foo.txt", "foo");
...
TEST_F(ArduinoFs, UnsuccessfulRename) {
  CreateTextFile("/a/b/c/foo.txt", "foo");
  ...
  // kNotDirectory would have been nicer.
  EXPECT_EQ(kNotFound, mount().rename("/a/b/c/foo.txt/e", "/x"));
...
TEST_F(ArduinoFs, ListOneElemDir) {
...
TEST_F(ArduinoFs, SuccessfullyReadFile) {
...
TEST_F(ArduinoFs, UnsuccessfulFopen) {
  CreateTextFile("/a/b/foo.txt", "foo");
  ...
  // kNotDirectory would have been nicer.
  EXPECT_EQ(kNotFound, mount().fopen("/a/b/foo.txt/c")->status());
}

TEST_F(ArduinoFs, SuccessfulCreateFile) {
...
TEST_F(ArduinoFs, SuccessfulOverwriteFile) {
...
TEST_F(ArduinoFs, SuccessfulAppendToFile) {
  CreateTextFile("/a/b/foo.txt", "Previous contents");
  const char* contents = "New contents";
...
TEST_F(ArduinoFs, ReadSeekAndSkip) {
  CreateTextFile("/a/b/foo.txt", "This is my text file");
...
TEST_F(ArduinoFs, ReadStressTest) {
  ...
  RecursiveMkDir("/foo/bar");
  {
    std::unique_ptr<OutputStream> out =
        mount().fopenForWrite("/foo/bar/foo.test", kFailIfExists);
